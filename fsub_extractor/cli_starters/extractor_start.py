...
def get_parser():
    ...
    reg_dir_group.add_argument(
        "--reg-anat",
        "--reg_anat",
        help="Paths to T1 image and brain mask (NIfTIs) in DWI space, separated by a comma (no spaces). These are used to calculate a registration if DWI space is not aligned to FreeSurfer (e.g., a QSIPrep T1w image in ACPC space). This overrides --fs2dwi, --dwi2fs, and --reg-type.",
        type=validate_file,
        metavar=("/PATH/TO/T1.nii.gz,/PATH/TO/mask.nii.gz"),
    )
    reg_dir_group.add_argument(
        "--fs2dwi",
        help="Path to MRTrix-ready or ANTs/ITK-generated registration for mapping FreeSurfer-to-DWI space. This can be generated by first running with '--reg-anat'. Mutually exclusive with --dwi2fs.",
        type=validate_file,
