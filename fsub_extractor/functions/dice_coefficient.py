...
def calculate_dice_coefficient(
 ...
 """Uses tckmap to transform two input tck_files to nifti images and then calculate the dice overlap.
   ...
   subprocess.run('tckmap -template '+ref_anat+' '+tck_file1+' '+out_name1) 

   out_name2=tck_file2[-3]+'nii.gz'
   subprocess.run('tckmap -template '+ref_anat+' '+tck_file2+' '+out_name2)
   ...
   track1 = sum(sum(sum(data1 > 0)))
   track2 = sum(sum(sum(data2 > 0)))
